...
const StatusPill: React.FC<{ status: string }> = ({ status }) => {
  ...
  return (
    <span className={`px-2 py-0.5 rounded-full text-xs border ${styles[status] ?? 'bg-gray-500/20 text-gray-400 border-gray-500/30'}`}>
      {status}
    </span>
  );
};
...
const AdvancedQuantumDashboard: React.FC = () => {
  ...
  const Sidebar = () => (
    <div className={`${sidebarOpen ? 'w-64' : 'w-16'} bg-gray-900/95 backdrop-blur-xl border-r border-gray-700/50 transition-all duration-300 flex flex-col`}>
        ...
        <div className="flex items-center gap-3">
          ...
          {sidebarOpen && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <h1 className="font-bold text-lg bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">النظام الكمي</h1>
      ...
      <div className="p-3 border-t border-gray-700/50">
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="w-full flex items-center justify-center p-2 rounded-lg bg-gray-800/50 hover:bg-gray-700/50 transition-colors text-gray-400">
        ...
        <div className="flex items-center gap-4">
          ...
          {report && (
            <span className="text-xs text-gray-500">
              وقت التشغيل: {Math.floor(report.stats.uptime / 60)}د {Math.floor(report.stats.uptime % 60)}ث
            </span>
          )}
        </div>
    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'الإنتاجية', value: `${c.throughput.toFixed(0)}`, unit: 'عملية/ث', color: '#3b82f6', icon: <Zap className="w-5 h-5" />, data: report.history.throughput },
            { label: 'زمن الاستجابة', value: `${c.latency.toFixed(1)}`, unit: 'ms', color: '#10b981', icon: <Clock className="w-5 h-5" />, data: report.history.latency },
            ...
            >
              ...
              <div className="text-2xl font-bold">{kpi.value}<span className="text-sm text-gray-400 mr-1">{kpi.unit}</span></div>
  ...
  const renderQuantum = () => {
    ...
    const algoInfo = typeof selectedAlgo === 'function'
      ? (selectedAlgo as (n?: number) => { name: string; description: string; qubits: number; gates: GateOp[] })()
      : selectedAlgo;
          ...
          {!showCustomCircuit ? (
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {algoList.map(algo => (
                  <button key={algo.key} onClick={() => setSelectedAlgorithm(algo.key)}
              ...
              <div className="mt-4 p-3 bg-gray-700/30 rounded-lg">
                ...
                <p className="text-xs text-gray-500 mt-1">{algoInfo.qubits} كيوبت • {algoInfo.gates.length} بوابة</p>
              </div>
            </>
          ) : (
                ...
                <div className="flex flex-wrap gap-2">
                  {customGates.map((g, i) => (
                    <span key={i} className="px-2 py-1 bg-purple-600/20 border border-purple-500/30 rounded text-xs text-purple-300">
                      {g.gate}({g.targets.join(',')}{g.controls ? ` ctrl:${g.controls.join(',')}` : ''})
                    </span>
                  ))}
                  <button onClick={() => setCustomGates([])} className="px-2 py-1 text-xs text-red-400 hover:text-red-300">مسح</button>
                </div>
              )}
            </div>
          )}
        ...
        {simResult && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: 'الكيوبتات', value: simResult.numQubits, icon: <Atom className="w-4 h-4" /> },
                { label: 'البوابات', value: simResult.gateCount, icon: <Cpu className="w-4 h-4" /> },
                { label: 'العمق', value: simResult.circuitDepth, icon: <BarChart3 className="w-4 h-4" /> },
                { label: 'الوقت', value: `${simResult.executionTimeMs.toFixed(1)}ms`, icon: <Clock className="w-4 h-4" /> },
              ].map((stat, i) => (
                <div key={i} className="bg-gray-800/50 rounded-xl p-4 border border-gray-700/50 text-center">
                  <div className="text-gray-400 mb-1 flex items-center justify-center gap-1 text-xs">{stat.icon} {stat.label}</div>
                  ...
                  .map(([state, count]) => {
                    ...
                    return (
                      <div key={state} className="flex-1 flex flex-col items-center gap-1 min-w-0">
                        ...
                        <motion.div initial={{ height: 0 }} animate={{ height: `${heightPct}%` }} transition={{ duration: 0.6 }}
                          className="w-full bg-gradient-to-t from-blue-600 to-purple-500 rounded-t-sm min-h-[2px]"
                        />
                        <span className="text-[10px] text-gray-400 font-mono transform -rotate-45 origin-top-left mt-1">|{state}⟩</span>
                      </div>
                    );
                  })}
              </div>
            ...
            {simResult.numQubits > 1 && (
              <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50">
                ...
                <div className="flex justify-center">
                  <div className="inline-grid gap-1" style={{ gridTemplateColumns: `auto repeat(${simResult.numQubits}, 40px)` }}>
                    <div />
                    {Array.from({ length: simResult.numQubits }, (_, i) => (
                      <div key={i} className="text-center text-xs text-gray-400 font-mono">Q{i}</div>
                    ))}
                    {simResult.entanglementMap.map((row, i) => (
                      <React.Fragment key={i}>
                        <div className="text-xs text-gray-400 font-mono flex items-center pr-2">Q{i}</div>
                        {row.map((entangled, j) => (
                          <div key={j} className={`w-10 h-10 rounded flex items-center justify-center text-xs ${
  ...
  const renderAI = () => {
    ...
    const c = report.current;
    const models = [
      { name: 'NLP-Advanced', type: 'NLP', accuracy: 96.8, status: 'running', load: c.aiModelLoad, memory: 3.7, gpu: 78 },
      { name: 'Vision-Quantum', type: 'CV', accuracy: 94.2, status: 'running', load: c.aiModelLoad * 0.8, memory: 5.2, gpu: 85 },
      { name: 'Quantum-ML', type: 'QML', accuracy: 91.5, status: 'training', load: c.aiModelLoad * 1.1, memory: 4.1, gpu: 92 },
      { name: 'Reinforcement-Agent', type: 'RL', accuracy: 88.3, status: 'running', load: c.aiModelLoad * 0.6, memory: 2.8, gpu: 45 },
    ];
    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gray-800/50 rounded-xl p-5 border border-gray-700/50">
            <div className="flex items-center gap-3 mb-3">
              <div className="p-2 rounded-lg bg-cyan-500/20"><Brain className="w-5 h-5 text-cyan-400" /></div>
              <div>
                <div className="text-2xl font-bold">{models.filter(m => m.status === 'running').length}/{models.length}</div>
          ...
          <div className="bg-gray-800/50 rounded-xl p-5 border border-gray-700/50">
            <div className="flex items-center gap-3 mb-3">
              <div className="p-2 rounded-lg bg-green-500/20"><Cpu className="w-5 h-5 text-green-400" /></div>
              <div>
                <div className="text-2xl font-bold">{(models.reduce((s, m) => s + m.gpu, 0) / models.length).toFixed(0)}%</div>
          ...
          <div className="bg-gray-800/50 rounded-xl p-5 border border-gray-700/50">
            <div className="flex items-center gap-3 mb-3">
              <div className="p-2 rounded-lg bg-purple-500/20"><TrendingUp className="w-5 h-5 text-purple-400" /></div>
              <div>
                <div className="text-2xl font-bold">{(models.reduce((s, m) => s + m.accuracy, 0) / models.length).toFixed(1)}%</div>
                ...
                {[
                  ...
                  <div key={j}>
                    ...
                    <div className="w-full bg-gray-700/50 rounded-full h-1.5">
                      <motion.div initial={{ width: 0 }} animate={{ width: `${metric.value}%` }} transition={{ duration: 1 }}
                        className="h-1.5 rounded-full" style={{ backgroundColor: metric.color }}
                      />
                    </div>
                  </div>
                ))}
        ...
        <TimeSeriesChart
          data={report.history.quantumUtilization.map((_p, i) => ({ value: 85 + Math.sin(i * 0.3) * 5 + (Math.random() - 0.5) * 2 }))}
          label="أداء AI الإجمالي" color="#06b6d4"
        />
      </div>
    );
  };
  ...
  const renderSecurity = () => {
    ...
    const threats = [
      { type: 'DDoS', severity: 'medium' as const, status: 'محجوب', count: 23, source: '185.x.x.x' },
      { type: 'SQL Injection', severity: 'high' as const, status: 'محجوب', count: 7, source: '45.x.x.x' },
      { type: 'Brute Force', severity: 'low' as const, status: 'محجوب', count: 156, source: 'متعدد' },
      { type: 'Quantum Attack', severity: 'critical' as const, status: 'تم الصد', count: 1, source: 'مجهول' },
    ...
    return (
      <div className="space-y-6">
        ...
        <TimeSeriesChart data={report.history.securityScore} label="درجة الأمان عبر الزمن" color="#10b981" />
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {threats.map((threat, i) => (
                  <tr key={i} className="border-t border-gray-700/30 hover:bg-gray-700/20 transition-colors">
                    <td className="py-3 px-4 font-medium">{threat.type}</td>
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 rounded-full text-xs border ${severityColor[threat.severity]}`}>{threat.severity}</span>
                    </td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-gray-800/50 rounded-xl p-5 border border-gray-700/50">
            <h4 className="font-semibold mb-4 flex items-center gap-2"><Lock className="w-4 h-4 text-blue-400" /> بروتوكولات التشفير</h4>
            <div className="space-y-2">
              {[
                ...
                <div key={i} className="flex items-center justify-between p-2 bg-gray-700/20 rounded-lg">
                  <div className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-400" /><span className="text-sm">{proto.name}</span></div>
          ...
          <div className="bg-gray-800/50 rounded-xl p-5 border border-gray-700/50">
            <h4 className="font-semibold mb-4 flex items-center gap-2"><Shield className="w-4 h-4 text-green-400" /> طبقات الحماية</h4>
            <div className="space-y-2">
              {['جدار الحماية المتقدم', 'كشف التسلل (IDS/IPS)', 'المصادقة الكمية', 'فحص الحزم العميق', 'العزل الكمي'].map((name, i) => (
                <div key={i} className="flex items-center justify-between p-2 bg-gray-700/20 rounded-lg">
                  <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-green-400" /><span className="text-sm">{name}</span></div>
    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            { label: 'المعالج (CPU)', value: c.cpu, color: '#3b82f6', icon: <Cpu className="w-5 h-5" />, data: report.history.cpu },
            { label: 'الذاكرة (RAM)', value: c.memory, color: '#10b981', icon: <Database className="w-5 h-5" />, data: report.history.memory },
            { label: 'التخزين', value: c.disk, color: '#f59e0b', icon: <HardDrive className="w-5 h-5" />, data: [] },
            { label: 'الشبكة', value: c.network, color: '#8b5cf6', icon: <Wifi className="w-5 h-5" />, data: report.history.network },
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-xl font-bold" style={{ color: metric.color }}>{metric.value.toFixed(1)}%</span>
              </div>
              <div className="w-full bg-gray-700/50 rounded-full h-2 mb-2">
                <motion.div initial={{ width: 0 }} animate={{ width: `${Math.min(100, metric.value)}%` }} transition={{ duration: 1 }}
        ...
        <div className="bg-gray-800/50 rounded-xl p-6 border border-gray-700/50">
          <h3 className="text-lg font-semibold mb-4 flex items-center gap-2"><Clock className="w-5 h-5 text-yellow-400" /> إحصائيات زمن الاستجابة</h3>
        ...
        {report.anomalies.length > 0 && (
          <div className="bg-gray-800/50 rounded-xl border border-gray-700/50 overflow-hidden">
            ...
            <div className="divide-y divide-gray-700/30 max-h-64 overflow-y-auto">
              {report.anomalies.slice(-10).reverse().map(a => (
                <div key={a.id} className="px-4 py-3 flex items-center justify-between hover:bg-gray-700/20">
                  <div className="flex items-center gap-3">
    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            {selected ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-gray-800/50 rounded-xl p-5 border border-gray-700/50">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    { label: 'النطاق الترددي', value: `${selected.metrics.bandwidth.toFixed(0)} Mbps` },
                    { label: 'فقد الحزم', value: `${(selected.metrics.packetLoss * 100).toFixed(3)}%` },
          ...
          {(['all', 'debug', 'info', 'warn', 'error', 'critical'] as const).map(level => (
            <button key={level} onClick={() => setLogFilter(level)}
              className={`px-3 py-1.5 rounded-lg text-xs transition-all ${
                logFilter === level ? 'bg-blue-600 text-white' : 'bg-gray-700/50 text-gray-400 hover:bg-gray-700 hover:text-white'
              }`}
            >
        ...
        <div className="bg-gray-800/50 rounded-xl border border-gray-700/50 overflow-hidden">
          <div className="divide-y divide-gray-700/30 max-h-[600px] overflow-y-auto font-mono text-sm">
            {logs.map(log => (
              ...
              >
                <span className="text-xs text-gray-500 flex-shrink-0 w-20 mt-0.5">{new Date(log.timestamp).toLocaleTimeString('ar-SA')}</span>
                <span className={`px-2 py-0.5 rounded text-xs flex items-center gap-1 flex-shrink-0 ${levelColors[log.level]}`}>
            ...
            {logs.length === 0 && <div className="p-8 text-center text-gray-500">لا توجد سجلات بالمرشح المحدد</div>}
          </div>
        </div>
      </div>
    );
  ...
  const renderSettings = () => (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="bg-gray-800/50 rounded-xl p-6 border border-gray-700/50">
        <h3 className="text-lg font-semibold mb-4 flex items-center gap-2"><Settings className="w-5 h-5 text-gray-400" /> إعدادات النظام</h3>
        <div className="space-y-4">
          {[
            ...
            <div key={i} className="flex items-center justify-between p-3 bg-gray-700/20 rounded-lg">
              <div>
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'analytics': return (
        <AIAnalysisDashboard
          systemMetrics={report?.current ? {
            cpu: report.current.cpu, memory: report.current.memory, disk: report.current.disk,
            network: report.current.network, uptime: `${Math.floor(report.stats.uptime / 60)}m`,
            activeConnections: report.current.activeConnections,
          } : null}
          quantumStates={[
            { id: 'q1', name: 'Quantum Alpha', status: 'active', qubits: 127, coherenceTime: 100.5, fidelity: 99.2, gateErrors: 0.001, temperature: 0.015 },
            { id: 'q2', name: 'Quantum Beta', status: 'idle', qubits: 64, coherenceTime: 85.3, fidelity: 98.7, gateErrors: 0.002, temperature: 0.018 },
          ]}
          securityMetrics={report?.current ? {
            encryptionLevel: 'AES-256 + Post-Quantum',
            threatLevel: report.current.securityScore > 95 ? 'low' : 'medium',
            activeThreats: 0, blockedAttacks: 199, quantumResistance: 99.9,
          } : null}
        />
      );
