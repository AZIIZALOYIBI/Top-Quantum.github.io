...
const PricingPage: React.FC = () => {
  ...
  const comparisons = [
    { category: 'الحوسبة الكمية', icon: <Atom className="w-5 h-5 text-purple-400" />, items: [
    ...
    { category: 'الذكاء الاصطناعي', icon: <Brain className="w-5 h-5 text-blue-400" />, items: [
    ...
    { category: 'الأمان', icon: <Shield className="w-5 h-5 text-green-400" />, items: [
    ...
    { category: 'الدعم والبنية التحتية', icon: <Headphones className="w-5 h-5 text-yellow-400" />, items: [
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <nav className="px-6 py-4 border-b border-gray-800">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/login')} className="px-4 py-2 text-sm text-gray-300 hover:text-white transition-colors">تسجيل الدخول</button>
            <button onClick={() => navigate('/register')} className="px-5 py-2 bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg text-sm font-semibold">ابدأ مجاناً</button>
          </div>
        </div>
      </nav>
      ...
      <section className="py-20 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-5xl font-bold mb-4">
            خطط <span className="bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">تناسب الجميع</span>
          </h1>
      ...
      <section className="max-w-3xl mx-auto px-6 pb-24">
        ...
        {[
          { q: 'هل يمكنني التبديل بين الخطط؟', a: 'نعم، يمكنك الترقية أو التخفيض في أي وقت. سيتم احتساب الفرق تلقائياً.' },
          { q: 'هل تقدمون فترة تجربة مجانية؟', a: 'نعم، جميع الخطط المدفوعة تتضمن 14 يوم تجربة مجانية بدون بطاقة ائتمان.' },
          { q: 'ما هي طرق الدفع المقبولة؟', a: 'نقبل بطاقات الائتمان (Visa, Mastercard, Amex)، التحويل البنكي، وPayPal للخطط المؤسسية.' },
          { q: 'هل البيانات آمنة؟', a: 'نستخدم تشفير ما بعد الكم (Kyber-1024, CRYSTALS-Dilithium) مع تشفير AES-256 في السكون والنقل.' },
          { q: 'هل يمكنني استخدام المنصة مع أجهزة IBM Quantum الحقيقية؟', a: 'نعم، خطة المؤسسات تتضمن تكامل مباشر مع IBM Quantum وAWS Braket.' },
        ].map((faq, i) => (
          <details key={i} className="group bg-gray-900/50 border border-gray-800 rounded-xl mb-3 overflow-hidden">
      ...
      <footer className="bg-gray-900 border-t border-gray-800 py-8">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between text-sm text-gray-500">
          ...
          <button onClick={() => navigate('/')} className="flex items-center gap-1 hover:text-white transition-colors mt-4 md:mt-0">
